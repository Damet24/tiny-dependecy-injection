import { ArgumentTypeNotExistsError, EnvironmentVariableNotExistsError, ParseEnvironmentVariableError } from './errors.js'
...
export class Argument {
    ...
    resolveEnvVar() {
        var result = this.value.match(/\$env\((.*?)\)/)
        if (result !== null) {
            const envVar = process.env[result[1]]
            if(envVar) return envVar;
            throw new EnvironmentVariableNotExistsError(result[1])
        } else {
            throw new ParseEnvironmentVariableError()
        }
    }
}
